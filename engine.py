...
class Player(object):
    def __init__(self, x, y):
        ...
        self.spell_launcher_image = pygame.image.load('Data/Images/Player/spell_launcher.png').convert_alpha()
        self.x = x
        self.y = y
        self.orig_y = 0
    ...
    def inventory(self, scroll, sorcers, archer_towers, archers, witch_huts, witches, surface):
        if self.selection == 0:
            ...
            self.spell_launcher(scroll, sorcers, archer_towers, archers, witch_huts, witches, surface)
        ...
        self.spell(surface, scroll, sorcers, archer_towers, archers, witch_huts, witches)
    ...
    def spell_launcher(self, scroll, sorcers, archer_towers, archers, witch_huts, witches, surface):
        mx, my = pygame.mouse.get_pos()
        spell_launcher_rect = pygame.Rect(self.rect.x + 6, self.rect.y + 16, 18, 8)
        spell_launcher_image = self.spell_launcher_image
        closest_enemy = None
        try:
            closest_sorcer = sorcers[0]
            closest_witch = witches[0]
            closest_hut = witch_huts[0]
            closest_archer = archers[0]
            closest_archer_tower = archer_towers[0]
        except IndexError:
            pass

        for sorcer in sorcers:
            sorcer_rect = pygame.Rect(sorcer[0], sorcer[1], 22, 27)
            closest_sorcer_rect = pygame.Rect(closest_sorcer[0], closest_sorcer[1], 22, 27)
            if self.rect.centerx - sorcer_rect.centerx < 250 and self.rect.centerx - sorcer_rect.centerx > -250 and self.rect.centery - sorcer_rect.centery < 250 and self.rect.centery - sorcer_rect.centery > -250:
                cs_side1 = abs(closest_sorcer_rect.centerx - self.rect.centerx)
                cs_side2 = abs(closest_sorcer_rect.centery - self.rect.centery)
                cs_hyp = math.sqrt(cs_side1**2 + cs_side2**2)
                s_side1 = abs(sorcer_rect.centerx - self.rect.centerx)
                s_side2 = abs(sorcer_rect.centery - self.rect.centery)
                s_hyp = math.sqrt(s_side1**2 + s_side2**2)
                if cs_hyp > s_hyp:
                    closest_sorcer = sorcer

        try:
            closest_enemy = closest_sorcer
            opp = math.degrees(math.atan2(closest_enemy[1], spell_launcher_rect.x))
            adj = math.degrees(math.atan2(closest_enemy[0], spell_launcher_rect.y))
            if pygame.mouse.get_pressed()[0]:
                self.spells.append([spell_launcher_rect.x, spell_launcher_rect.y, [adj, opp]])
        except UnboundLocalError:
            pass

        if mx >= self.rect.centerx:
            spell_launcher_image = self.spell_launcher_image
        if mx < self.rect.centery:
            spell_launcher_image = pygame.transform.flip(self.spell_launcher_image, True, False)
        surface.blit(spell_launcher_image, (spell_launcher_rect.x - scroll[0], spell_launcher_rect.y - scroll[1], spell_launcher_rect.width, spell_launcher_rect.height))

    def spell(self, surface, scroll, sorcers, archer_towers, archers, witch_huts, witches):
        spell_image = pygame.image.load('Data/Images/Player/spell.png')
        for spell in self.spells:
            spell[0] += math.cos(spell[2][0]) * 6
            spell[1] += math.sin(spell[2][1]) * 6
            surface.blit(spell_image, (spell[0] - scroll[0], spell[1] - scroll[1], 5, 5))
