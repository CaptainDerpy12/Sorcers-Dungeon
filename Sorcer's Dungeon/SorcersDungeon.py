...
while running:
    ...
    if game_state['game_running']:
        ...
        tile_list = levelRenderer.renderLevel(display, level_map, tile_list, pathfind_tiles, spawners, archer_tower, archer_towers, archers, new_level, scroll)
        new_level = False

        # Player Stuff
        player.move_player(tile_list, scroll)
        player.render(display, scroll, player_health)
        player.inventory(scroll, sorcers_list, archer_towers, archers, display)
        ...
        #     if pygame.Rect(arrow_x, arrow_y, 6, 1).colliderect(player.rect):
        #         archer_arrows.remove(arrow)
        #         screen_shake = 10
        #         if player.x > arrow_x:
        #             player.x += 10
    ...
    render_offset = [0, 0]
    if screen_shake:
        render_offset[0] = random.randint(-6, 6)
        render_offset[1] = random.randint(-6, 6)
